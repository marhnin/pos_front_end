...
@Component({
  selector: 'app-user',
  templateUrl: './promoter-commission-report.component.html',
  styleUrls: ['./promoter-commission-report.component.css'],
  providers: [DatePipe]

})
export class PromoterCommissionReportComponent implements OnInit {
  fromdate: string ='';
  todate: string = '';
  section: string= '';
...
exportexcel(): void 
    ...
    this.filename = "promoter_"+this.filename_date;
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', this.dto.token);
    ...
    formData.append("agentId", this.agentId);
     if(this.report_option == 'excel')
      ...
      this.http.post(this.funct.ipaddress + 'promoterreport/promotercommissionreport', formData, { headers: headers})
      .pipe(
      ...
      .subscribe(data => {
        ...
        {
                var title = '';
                title = 'Promoter Commission Report ('+this.formonth+')';
                const header = ['No', 'Name', 'Phone Number', 'Referral Code',  'Bet Date', 'Type', 'Bet Amount'];
                const workbook = new Workbook();
                const worksheet = workbook.addWorksheet('Promoter Commission Report');
        ...
        if(this.report_option == 'csv')
        {
          ...
          this.toastr.success('Promoter Commission Report success', 'Success!', {
            timeOut: 3000,
            positionClass: 'toast-top-right',
          });
        this.spinner.hide();
  ...
  convert(list, pdfname){
      ...
      const head = [['No', 'Name', 'Phone Number', 'Referral Code', 'Bet Date','Type',  'Bet Amount']];
      doc.setFontSize(12);
      doc.setTextColor(0);
      var tot = 0;
      var title = '';
      ...
      downloadFile(data, filename) {
        let csvData = this.ConvertToCSV(data, ['sr_no','name', 'phone_no','referral_code','type','betdate_Str','total_amount_Str']);
        console.log(csvData)//'\ufeff' +
