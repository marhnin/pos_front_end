...
@Component({
  selector: 'app-twod-bet-amount-limitation-detail-update',
  templateUrl: './twod-bet-amount-limitation-detail-update.component.html',
  styleUrls: ['./twod-bet-amount-limitation-detail-update.component.css']
})
export class TwodBetAmountLimitationDetailUpdateComponent implements OnInit {
  amount: string = '';
  singleDate: string = '';
  time: string = '';
  ...
  getBetLimitById() {
    ...
    this.http.get(this.funct.ipaddress + 'betamountLimitation/GetDetailList', { params: params, headers: headers })
    .pipe(
    ...
    .subscribe(
      result => {
        ...
        this.twodbetDTO = this.dto.Response;
        this.for_date = new Date(this.twodbetDTO.date);
  ...
  editBetLimit()
  {
      ...
      let headers = new HttpHeaders();
      headers = headers.set('Authorization',  this.token);
      this.http.post(this.funct.ipaddress + 'betamountLimitation/update', this.twodbetDTO, { headers: headers })
      .pipe(
        catchError(this.handleError.bind(this))
       )
      .subscribe(
