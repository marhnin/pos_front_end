...
@Component({
  selector: 'app-user',
  templateUrl: './handle-amount-beforeclean-report.component.html',
  styleUrls: ['./handle-amount-beforeclean-report.component.css'],
  providers: [DatePipe]

})
export class AccountHolderBeforeCleanReportComponent implements OnInit {
  fromdate: string ='';
  todate: string = '';
  status: string= '';
  type: string= '';
 ...
 onChangeCode()
  {
   ...
   bankAccountDetailReport(){
    ...
    this.filename = "bankAccountDetail_beforeclean_"+this.filename_date;
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', this.dto.token);
    ...
    formData.append("codeNumbes",this.codeoptionsModel.toString())
     if(this.report_option == 'excel')
      ...
      this.http.post(this.funct.ipaddress + 'userfinancialreport/bankaccountdetailreportBeforeCleanHistory', formData, { headers: headers})
      .pipe(
      ...
      if(this.report_option == "excel")
      {
      const title = 'Bank Account Detail Report (Before Clean)';
      const header = ['No', 'Date','Account Holder (Admin)', 'Code Number','Account Name','Account Number','Bank Type','Handel Amount'];
      const workbook = new Workbook();
      const worksheet = workbook.addWorksheet('Bank Account Detail Report');
   ...
   if(this.report_option == 'csv')
   {
      this.downloadFile(data, this.filename+repType)
            ...
            this.toastr.success('Bank Account Detail Reporting', 'Success!', {
              timeOut: 3000,
              positionClass: 'toast-top-right',
            });
          this.spinner.hide();
  ...
  convert(list, pdfname){
    ...
    const head = [['No', 'Date','Account Holder (Admin)', 'Code Number', 'Account Name','Account Number','Bank Type','Handel Amount']];
    // define custom font
    doc.setFontSize(12);
    doc.setTextColor(0);
    doc.text('Bank Account Detail Report', 40, 20);
    ...
    downloadFile(data, filename) {
      let csvData = this.ConvertToCSV(data, ['sr_no', 'date', 'acc_holder', 'code_number','acc_name','acc_number','bank_name','my_amt_Str']);
      console.log(csvData)
