...
@Component({
  selector: 'app-user',
  templateUrl: './handle-amount-report.component.html',
  styleUrls: ['./handle-amount-report.component.css'],
  providers: [DatePipe]

})
export class AccountHolderReportComponent implements OnInit {
  fromdate: string ='';
  todate: string = '';
  status: string= '';
  type: string= '';
 ...
 onChangeCode()
  {
   ...
   bankAccountDetailReport(){
    ...
    this.filename = "bankAccountDetail_"+this.filename_date;
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', this.dto.token);
    ...
    formData.append("codeNumbes",this.codeoptionsModel.toString())
     if(this.report_option == 'excel')
      ...
      this.http.post(this.funct.ipaddress + 'userfinancialreport/bankaccountdetailreport', formData, { headers: headers})
      .pipe(
      ...
      if(this.report_option == "excel")
      {
      const title = 'Bank Account Detail Report';
