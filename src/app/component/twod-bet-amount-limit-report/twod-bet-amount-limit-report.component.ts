...
@Component({
  selector: 'app-user',
  templateUrl: './twod-bet-amount-limit-report.component.html',
  styleUrls: ['./twod-bet-amount-limit-report.component.css'],
  providers: [DatePipe]

})
export class TwoBetLimitReportComponent implements OnInit {
  fromdate: string ='';
  todate: string = '';
  section: string= '';
...
exportexcel(): void 
   ...
   twodReport(){
    ...
    this.filename = "twoDBetLimit"+section+"_"+this.filename_date;
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', this.dto.token);
    let params = new HttpParams();
    params = params.set('fordate',this.fromdate).set('section',this.section).set('type', '2D');
     if(this.report_option == 'excel')
      ...
      this.http.get(this.funct.ipaddress + 'betamountLimitation/betAmountLimitReport', { params: params, headers: headers})
      .pipe(
      ...
      .subscribe(data => {
        ...
        if(this.report_option == "pdf")
        {
         ...
         // this.twodBetPdfRptList.push(totalList)
          this.convert(this.twodBetRptList,pdfname);
        }
        if(this.report_option == "excel")
        {
                ...
                var title = '';
                if(this.section == ''){
                   title = 'TwoD Bet Amount Limit Report (All)';
                }
                else
                   title = 'TwoD Bet Amount Limit Report ('+this.section+')';
                const header = ['No', 'Bet Date', 'Bet Time', '2D Number', 'Limited Amount', 'Total Bet Amount', 'Different Amount'];
                // Create workbook and worksheet
                const workbook = new Workbook();
                const worksheet = workbook.addWorksheet('TwoD Bet Amount Limit Report');
  ...
  convert(list, pdfname){
      ...
      const head = [['No', 'Bet Date', 'Bet Time', '2D Number', 'Limited Amount', 'Total Bet Amount', 'Different Amount']];
      doc.setFontSize(12);
      doc.setTextColor(0);
      var tot = 0;
      var rows1 = [];
      ...
      var title = '';
      if(this.section == ''){
         title = '2D Bet Bet Amount Limit Report (All)';
      }
      else
         title = '2D Bet Bet Amount Limit Report ('+this.section+')';
      ...
      downloadFile(data, filename) {
        let csvData = this.ConvertToCSV(data, ['sr_no', 'bet_date_Str','bet_number','section','limitAmt_Str','total_amt_Str','different_amt']);
        console.log(csvData)//'\ufeff' +
