...
@Component({
  selector: 'app-user',
  templateUrl: './threed-bet-amount-limit-report.component.html',
  styleUrls: ['./threed-bet-amount-limit-report.component.css'],
  providers: [DatePipe]

})
export class ThreedBetLimitReportComponent implements OnInit {
  fromdate: string ='';
  todate: string = '';
  section: string= '';
...
exportexcel(): void 
   ...
   twodReport(){
    ...
    this.filename = "threeDBetLimit_"+this.filename_date;
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', this.dto.token);
    let params = new HttpParams();
    params = params.set('fromdate',this.fromdate).set('todate',this.todate);
     if(this.report_option == 'excel')
      ...
      this.http.get(this.funct.ipaddress + 'betamountLimitation/3dbetAmountLimitReport', { params: params, headers: headers})
      .pipe(
      ...
      .subscribe(data => {
        ...
        if(this.report_option == "excel")
        {
                ...
                var title = '';
                   title = 'ThreeD Bet Amount Limit Report';
                const header = ['No', 'Bet Date', '3D Number', 'Limited Amount', 'Total Bet Amount', 'Different Amount'];
                // Create workbook and worksheet
                const workbook = new Workbook();
                const worksheet = workbook.addWorksheet('ThreeD Bet Amount Limit Report');
        ...
        if(this.report_option == 'csv')
        {
          ...
          this.toastr.success('3D Bet Limit Report success', 'Success!', {
            timeOut: 3000,
            positionClass: 'toast-top-right',
          });
        this.spinner.hide();
  ...
  convert(list, pdfname){
      ...
      const head = [['No', 'Bet Date', '3D Number', 'Limited Amount', 'Total Bet Amount', 'Different Amount']];
      doc.setFontSize(12);
      doc.setTextColor(0);
      var tot = 0;
      var rows1 = [];
      rows1.push("Total = "+40);
      var title = '';
      title = '3D Bet Bet Amount Limit Report';
      ...
      downloadFile(data, filename) {
        let csvData = this.ConvertToCSV(data, ['sr_no', 'bet_date_Str','bet_number','limitAmt_Str','total_amt_Str','different_amt']);
        console.log(csvData)//'\ufeff' +
