...
@Component({
  selector: 'app-user',
  templateUrl: './twodbet35percentreport.component.html',
  styleUrls: ['./twodbet35percentreport.component.css'],
  providers: [DatePipe]

})
export class Twod35PercentReportComponent implements OnInit {
  fromdate: string ='';
  todate: string = '';
  section: string= '';
    ...
    if($("#foo_bar").is(':checked'))
     {
         ...
         this.htitle = "TwoD Bet History Report (35%)";
      ...
      .subscribe(data => {
        ...
        if(this.report_option == "excel")
        {
                ...
                if(this.fifty50percent == 'checked'){
                   this.generateExcelOnly35Percent(repType,this.twodBetRptList );
                   return;
                }
...
generateExcelOnly35Percent(repType, list) /*for 35% only*/
{
  console.log("dkdkdd" +list)
          // Excel Title, Header, Data
          var title = '';
          if(this.section == ''){
            title = 'TwoD Bet History Report Only 35% (All)';
          }
          else
            title = 'TwoD Bet History Report Only 35% ('+this.section+')';
  ...
  convert(list, pdfname){
      ...
      convert1(list, pdfname){
        ...
        var title = '';
        if(this.section == ''){
           title = '2D Bet History Report Only 35% (All)';
        }
        else
           title = '2D Bet History Report Only 35% ('+this.section+')';
        doc.text(title, 40, 20);
        doc.text('Printed By : '+this.ad_name+' - Printed Date : '+this.filename_date,40,40);
        autoTable(doc,
        {
