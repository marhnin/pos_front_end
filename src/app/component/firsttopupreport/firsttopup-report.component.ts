...
@Component({
  selector: 'app-user',
  templateUrl: './firsttopup-report.component.html',
  styleUrls: ['./firsttopup-report.component.css'],
  providers: [DatePipe]

})
export class FirstTopupReportComponent implements OnInit {
  fromdate: string ='';
  todate: string = '';
  section: string= '';
...
exportexcel(): void 
    ...
    this.filename = "firsttopup"+this.filename_date;
    let headers = new HttpHeaders();
    headers = headers.set('Authorization', this.dto.token);
    ...
    formData.append("agentId", this.agentId);
     if(this.report_option == 'excel')
      ...
      this.http.post(this.funct.ipaddress + 'userfinancialreport/userfirsttopupReport', formData, { headers: headers})
      .pipe(
      ...
      .subscribe(data => {
        ...
        {
                var title = '';
                   title = 'First Topup Report Monthly('+this.formonth+')';
